...
import static com.bvutest.core.pwa.MainActivity.main;
...
class PwaWebViewClient extends WebViewClient {
    ...
    private ArrayList<String> adUrls = new ArrayList<>(Arrays.asList("https://www.iherb.com/cl/new-brands"));
...
//        List<String> blockIdList = Arrays.asList("Buenos Aires", "Córdoba", "La Plata");
    ...
    public void loadAdOnUrl(String url){
        try{
            for(String item : adUrls){
                Log.d(TAG, "loadAdOnUrl >> " + url + " | " + item);
                if(url.contains(item)) {
                    Log.d(TAG, "loadAdOnUrl == match >> load ad");
                    main.loadInterstitialAd();
                }
            }
        }catch(Exception e){
            Log.d(TAG, "loadAdOnUrl error >> "+e);
        }
    }
